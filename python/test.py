...
class FrankaRobotPublisher (Node):
    def __init__(self):
        ...
        spheres = Obstacles([
            Obstacle([0.0 ,0.5, 0.3], 0.3),
            Obstacle([0.3 ,-0.5, 0.3], 0.3)
            # Obstacle([0.0 ,-0.075, 0.01], 0.18)
            ],
            obstacle_type=Marker.SPHERE)
        self.add_obstacle (
            frame_id = self.frame_id_base,
            obstacles = spheres
            ) 
    ...
    def timer_callback(self):
        ...
        # sinus_value = (math.sin(self.i/10) + 1 + 0)/2
        
        for ii, obs in enumerate(self.obstacles_array):
            print(ii)
